...
import { deleteMovie, removeMovieFromDatabase } from '../../manager/actions/movieAction';
    ...
    <Link className="btn btn-success my-4" to="/admin/add">Add movie</Link>  
      ...
      <tbody>
        {movies.map((item,count)=>(
          ...
          <td><Button onClick={()=>{dispatch(removeMovieFromDatabase(item.id))}} variant='danger'>X</Button></td>
        </tr>
        ))}
